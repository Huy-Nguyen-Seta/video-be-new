...
| `ENABLE_VIEW_FLUSHER` | `false` | bật cron flush view từ Redis xuống DB. **Phải set `true` ở đúng 1 process** (xem dưới) |
...
> Cái cron flush này phải bật ở **đúng một process** (`ENABLE_VIEW_FLUSHER=true`), nếu bật ở nhiều pod thì chúng cùng `getdel` đua nhau — vẫn không mất view (atomic) nhưng thừa. Trong `docker-compose` để **đúng một process API** giữ vai trò flush (worker không bật biến này).
